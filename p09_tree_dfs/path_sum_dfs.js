...
One of the first 2 implementations are probably the most intuitive to me, and likely
what I will come up with during a coding interview.

The idea is that, we want to subtract node value from remaining sum after we 
check that the node is not null, but before we check if node is a leaf node.
*/
...
With this implementation, we check if remaining sum equals 0 instead of node.val, 
and instead of a one liner return value in the dfs method, we have a boolean 
variable to store whether the left or right path are valid path sums.
*/
